
## bugs
    [x] dynamic zoom is not working if the app was started minimized


## enhancements
    [x] add snap revision in debug info if install type is snap package
    [x] add quick theme toggle action in system tray menu
    [x] add option in settings that let user set : lock app on minimize to tray
    [x] add ability to activate whatsie window using global shortcut
    [x] prevent reloading when app is locked
    [x] prevent new message option when app is locked
    [ ] embed the icon directly into the executable or application bundle.
